...
ノート pc で Windows 10 Hyper-v を実行していて、ワイヤレスネットワークと有線ネットワークを頻繁に切り替えている場合は、イーサネットとワイヤレスネットワークカードの両方に対して仮想スイッチを作成することをお勧めします。  ラップトップをネットワークに接続する方法に応じて、これらのスイッチの間で仮想マシンを変更することができます。 仮想マシンは有線とワイヤレスを自動的に切り替えることはできません。 

>[!IMPORTANT]
>この2つのスイッチアプローチでは、ワイヤレスカード経由の外部 vSwitch はサポートされておらず、テスト目的でのみ使用することをお勧めします。
