...
* **標準的なチェックポイント**: チェックポイントが開始時の仮想マシンのメモリ状態、仮想マシンのスナップショットを取得します。 スナップショットは完全なバックアップではなく、ノード間でデータをレプリケートするシステム (たとえば Active Directory) でのデータ整合性に関する問題の原因となることがあります。  Windows 10 より前の Hyper-V で提供されていたのは、標準のチェックポイントのみでした (以前はスナップショットと呼ばれていました)。

* **生産チェックポイント**: 仮想マシンの一貫性のあるデータのバックアップを作成するのには、Linux 仮想マシンのボリューム シャドウ コピー サービスまたはシステムを固定してファイルを使用します。 仮想マシンのメモリの状態のスナップショットは取得されません。
...
多くのチェックポイントは、決まった時点で作成されます。  識別名を追加すると、チェックポイントが作成されたときに、システムの状態の詳細を保存しやすきます。
...
1. まだを開き、仮想マシンのデスクトップから削除する場合は、テキスト ファイルを閉じます。
...
1. 仮想マシンにログオンし、新しいテキスト ファイルを作成します。 前の手順を実行している場合は、既存のテキスト ファイルを使用することができます。
2. 「これは運用チェックポイントです」と テキスト ファイルに入力して、このファイルを保存します。ただし、**メモ帳は閉じないでください**。
3. HYPER-V マネージャーを開く、仮想マシンを右クリックし、**チェックポイント**を選択します。
...
1. まだを開き、仮想マシンのデスクトップから削除する場合は、テキスト ファイルを閉じます。
2. HYPER-V マネージャーを開く、運用チェックポイントを右クリックし、**適用**] を選択します。
3. [チェックポイントの適用] の通知ウィンドウで **[適用]** を選択します。
