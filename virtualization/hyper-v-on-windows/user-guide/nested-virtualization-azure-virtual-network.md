...
## <a name="create-the-second-network-interface"></a>2番目のネットワークインターフェイスを作成する
...
3. 次のコマンドを実行し `Install-WindowsFeature -Name Hyper-V -IncludeManagementTools -Restart`
4. これにより、ホストが再起動されます
...
6. [NAT] を右クリックし、[新しいインターフェイス...] を選択します。[イーサネット] を選択します。これは、IP が "10.0.0.4" である最初の NIC であり、[パブリックインターフェイス] を選択してインターネットに接続し、このインターフェイスで NAT を有効にします。 
