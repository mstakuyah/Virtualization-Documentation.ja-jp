...
* ウィザードの次へ] を選択、および「ユーザー設定の構成」リング ボタンをオンに"次へ] を選択します。
* "NAT"チェック"LAN"とし、["次へ] と [「完了」します。 場合は、サービスを開始し、[実行することが表示されます。
* 今すぐ"IPv4"ノードに移動し、展開"NAT"ノードを利用できるようにします。
* "NAT"を右クリックし、[「新しいインターフェイス.」を選択する 3 つのオプションを表示する必要があります。 
* このオプションを選択、HYPER-V 仮想スイッチの次のオプションの 2 つは、「イーサネット」のオプションを表示する必要があります。 つまり、Azure VNet に直接接続されているインターフェイスを選択します。
