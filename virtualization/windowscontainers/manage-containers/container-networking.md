...
# Windows コンテナーのネットワークの詳細

## ネットワーク コンパートメントによる分離 (名前空間)
...
仮想フィルタリング プラットフォーム (VFP) 拡張機能は、Hyper-V 仮想スイッチの、ネットワーク ポリシーの適用とパケットの操作に使用されるフォワーディング拡張機能です。 たとえば、VFP を 'overlay' ネットワーク ドライバーで使用して VXLAN カプセル化を実行したり、'l2bridge' ドライバーで使用して入口と出口で MAC の再書き込みを実行したりすることができます。 VFP 拡張機能は、Windows Server 2016 および Windows 10 Creators Update にのみ含まれています。 これが正しく実行されているかどうかを確認するには、次の 2 つのコマンドを実行します。
...
#### HNS では、コンテナーのホスト コンピューターで IPv6 が有効になっている必要がある 
...
#### Moby Linux VM は、Docker for Windows ([Docker CE](https://www.docker.com/community-edition) の製品) で、HNS の内部 vSwitch の代わりに、DockerNAT スイッチ を使用する 
...
#### 仮想コンテナー ホストでの IP の割り当てに DHCP を使用するには、MACAddressSpoofing を有効にする 
...
```
#### 1 つのコンテナー ホストで複数の透過ネットワークを作成する
...
#### 静的 IP 割り当てを使用する場合は、必ず *--subnet* と *--gateway* を指定する
...
#### DHCP による IP 割り当ては L2Bridge ネットワークでサポートされない
...
#### 既存の vSwitch (Docker には表示されない) が透過ネットワークの作成をブロックする場合がある
